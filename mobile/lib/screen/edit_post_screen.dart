...
class _EditPostScreenState extends State<EditPostScreen> {
  ...
  void _loadPostData() {
    ...
    _postType = post['post_type'] ?? 'regular';
    _visibility = post['visibility'] ?? 'public';

    if (_postType == 'event') {
      if (post['event_date'] != null) {
        try {
          _selectedEventDate = DateTime.parse(post['event_date']);
        } catch (_) {}
      }
      if (post['event_start_time'] != null) {
        final timeStr = post['event_start_time'] as String;
      ...
      if (post['event_end_time'] != null) {
        final timeStr = post['event_end_time'] as String;
        final parts = timeStr.split(':');
        if (parts.length >= 2) {
      ...
      _eventLatitudeController.text = post['event_latitude']?.toString() ?? '';
      _eventLongitudeController.text =
          post['event_longitude']?.toString() ?? '';
      _eventQuotaController.text = post['event_quota']?.toString() ?? '';
      _eventRegistrationLinkController.text =
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'Mengedit di ${widget.communityName}',
                        style: GoogleFonts.poppins(
                          fontSize: 13,
              ...
              _fieldLabel('Tambah Media (Opsional)'),
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 4),
                    Text(
                      'Media yang sudah ada akan tetap tersimpan',
                      style: GoogleFonts.poppins(
                        fontSize: 10,
                        color: Colors.grey.shade400,
